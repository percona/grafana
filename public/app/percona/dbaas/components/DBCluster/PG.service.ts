...
export class PGService extends DBClusterService {
  ...
  updateDBCluster(dbCluster: DBCluster): Promise<void | DBClusterPayload> {
    return apiManagement.post<void, DBClusterPayload>('/DBaaS/PostgresqlCluster/Update', toAPI(dbCluster));
  }
  ...
  resumeDBCluster(dbCluster: DBCluster): Promise<void | DBClusterPayload> {
    return apiManagement.post<DBClusterPayload, DBClusterSuspendResumeRequest>(
      '/DBaaS/PostgresqlCluster/Update',
  ...
  getDBClusterCredentials(dbCluster: DBCluster): Promise<void | DBClusterConnectionAPI> {
    return apiManagement.post<DBClusterConnectionAPI, Omit<DBClusterPayload, 'params'>>(
      '/DBaaS/PostgresqlCluster/GetCredentials',
  ...
  getComponents(kubernetesClusterName: string): Promise<DBClusterComponents> {
    return apiManagement.post<DBClusterComponents, object>('/DBaaS/Components/GetPG', {
  ...
  setComponents(kubernetesClusterName: string, componentsVersions: ManageComponentsVersionsRenderProps): Promise<void> {
    return apiManagement.post<void, DBClusterChangeComponentsAPI>('/DBaaS/Components/ChangePG', {
      kubernetes_cluster_name: kubernetesClusterName,
      postgresql: getComponentChange(Operators.pg, SupportedComponents.postgresql, componentsVersions),
  ...
  getDatabaseVersions(kubernetesClusterName: string): Promise<DatabaseVersion[]> {
    return this.getComponents(kubernetesClusterName).then(({ versions }) => {
      return Object.entries(versions[0].matrix.postgresql || {}).map(([version, component]) => ({
  ...
  getExpectedResources(dbCluster: DBCluster): Promise<DBClusterExpectedResources> {
    return apiManagement
      .post<DBClusterExpectedResourcesAPI, Partial<DBClusterPayload>>(
        '/DBaaS/PostgresqlCluster/Resources/Get',
  ...
  toModel(dbCluster: DBClusterPayload, kubernetesClusterName: string, databaseType: Databases): DBCluster {
    return {
      ...
      memory: (dbCluster.params.instance?.compute_resources?.memory_bytes || 0) / BILLION,
      cpu: (dbCluster.params.instance?.compute_resources?.cpu_m || 0) / THOUSAND,
      disk: (dbCluster.params.instance?.disk_size || 0) / BILLION,
